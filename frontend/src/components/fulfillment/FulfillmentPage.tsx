...
import { fulfillmentItems, fulfillmentOrder, recommendedAllocations, reservation, shipments, warehouseStock } from "./mockData";
...
const productName = (productId: string) => fulfillmentItems.find((item) => item.id === productId)?.name ?? productId;
const productSku = (productId: string) => fulfillmentItems.find((item) => item.id === productId)?.sku ?? "";
...
export function FulfillmentPage() {
  ...
  return (
    <>
      ...
      <div className="flex-1 h-screen flex flex-col overflow-hidden">
        ...
        <main className="flex-1 flex flex-col overflow-hidden bg-background">
          <div className="flex-1 overflow-y-auto px-space-xl py-space-lg space-y-space-base pb-32">
            <FulfillmentHeader status={fulfillmentStatus} />
            <OrderSummary status={fulfillmentStatus} />
            ...
            <div className="grid grid-cols-2 gap-space-base">
              <ReservationCard onView={() => setIsReservationOpen(true)} />
              <ShipmentPlan onView={setActiveShipment} />
            </div>
            <BackorderStatus />
      ...
      {isReservationOpen && <ReservationDialog onClose={() => setIsReservationOpen(false)} />}
...
function FulfillmentHeader({ status }: { status: FulfillmentStatus }) {
  return <>
    ...
    <div className="flex items-center justify-between pb-space-xs">
      <div className="flex items-center gap-3">
        <div>
          ...
          <p className="font-body-sm text-body-sm text-outline mt-0.5">Acme Corporation <span className="mx-1">·</span> From Quote #Q-1042</p>
        </div>
...
function OrderSummary({ status }: { status: FulfillmentStatus }) {
  const values = [
    ["Customer", fulfillmentOrder.customer], ["Source Quote", fulfillmentOrder.sourceQuote], ["Order Value", money(fulfillmentOrder.orderValue)], ["Physical Units", "15"], ["Fulfillment Status", status], ["Approval", fulfillmentOrder.approval], ["Sales Representative", fulfillmentOrder.salesRepresentative],
  ];
...
function FulfillmentItemsTable() {
...
function InventoryAvailability() {
  return <section className={`${card} overflow-hidden`}>
    ...
    <div className="overflow-x-auto"><table className="w-full text-left border-collapse min-w-[760px]"><thead><tr className="bg-[#F8F9FA] border-b border-[#E5E7EB] h-9 font-label-sm text-label-sm text-[#475569] uppercase tracking-wider"><th className="py-2.5 px-space-base font-semibold">Product</th><th className="py-2.5 px-space-md font-semibold">Warehouse</th><th className="py-2.5 px-space-md text-right font-semibold">Available</th><th className="py-2.5 px-space-md text-right font-semibold">Reserved</th><th className="py-2.5 px-space-md text-right font-semibold">Fulfillable</th><th className="py-2.5 px-space-base font-semibold">Status</th></tr></thead><tbody className="divide-y divide-[#F1F5F9] font-body-md text-body-md">{warehouseStock.map((stock) => { const status = stock.fulfillable === 0 ? "Unavailable" : stock.fulfillable < 4 ? "Limited" : "Available"; return <tr key={`${stock.productId}-${stock.warehouse}`} className="hover:bg-[#F8FAFC]"><td className="py-2.5 px-space-base"><span className="font-semibold text-on-surface">{productName(stock.productId)}</span><span className="font-code-tabular text-[11px] text-outline ml-2">{productSku(stock.productId)}</span></td><td className="py-2.5 px-space-md text-on-surface-variant">{stock.warehouse}</td><td className="py-2.5 px-space-md text-right font-code-tabular tnum">{stock.available}</td><td className="py-2.5 px-space-md text-right font-code-tabular tnum text-outline">{stock.reserved}</td><td className="py-2.5 px-space-md text-right font-code-tabular tnum font-bold text-on-surface">{stock.fulfillable}</td><td className="py-2.5 px-space-base"><span className={status === "Available" ? successBadge : status === "Limited" ? warningBadge : "inline-flex px-2.5 py-0.5 rounded-full text-label-sm font-semibold bg-[#F1F5F9] text-[#64748B] border border-[#E2E8F0]"}>{status}</span></td></tr>; })}</tbody></table></div>
  </section>;
}
...
function FulfillmentActionBar({ allocationUnits, shipping, draftSaved, onSaveDraft, onConfirm }: { allocationUnits: number; shipping: number; draftSaved: boolean; onSaveDraft: () => void; onConfirm: () => void }) { return <footer className="h-16 px-space-xl bg-white border-t border-[#E5E7EB] flex items-center justify-between flex-shrink-0 z-20 shadow-[0px_-4px_8px_rgba(15,23,42,0.03)]"><div className="flex items-center gap-3"><button onClick={onSaveDraft} className="h-9 px-4 rounded-md bg-white border border-[#D1D5DB] text-on-surface hover:bg-[#F9FAFB] font-label-md text-label-md font-semibold flex items-center gap-1.5 transition-colors shadow-sm"><span className="material-symbols-outlined text-sm text-outline">save</span>{draftSaved ? "Draft Saved" : "Save Draft"}</button>{draftSaved && <span className="font-body-sm text-body-sm text-[#065F46]">Saved locally for this session</span>}</div><div className="flex items-center gap-4"><div className="text-right"><span className="block font-label-sm text-[10px] uppercase tracking-wider text-outline">Estimated Shipping</span><span className="font-title-md text-body-md font-bold text-on-surface font-code-tabular tnum">{money(shipping)} <span className="text-outline font-normal">·</span> {allocationUnits} / 15 units</span></div><button onClick={onConfirm} className="h-10 px-5 rounded-md bg-primary hover:bg-[#1E3A8A] text-on-primary font-title-md text-body-md font-semibold flex items-center gap-2 shadow-sm transition-colors">Confirm Fulfillment Plan<span className="material-symbols-outlined text-base">send</span></button></div></footer>; }
...
function ReservationDialog({ onClose }: { onClose: () => void }) { return <Dialog title={`Reservation ${reservation.id}`} onClose={onClose}><div className="space-y-3 font-body-sm text-body-sm"><Detail label="Status" value="Reserved" /><Detail label="Reserved At" value={reservation.reservedAt} /><Detail label="Reservation Expires" value={reservation.expiresAt} /><div className="pt-3 border-t border-[#E5E7EB]"><span className="font-label-sm text-[10px] uppercase tracking-wider text-outline">Reserved quantities</span><p className="text-on-surface mt-1">Mumbai Warehouse: Laptop Pro 14 × 5, Studio Display × 3</p><p className="text-on-surface">Pune Warehouse: Laptop Pro 14 × 5, Studio Display × 2</p></div></div></Dialog>; }
